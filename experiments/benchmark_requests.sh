...
get_benchmark_data () {
  ...
  styx matplotlib --prometheus http://localhost:9090 --duration 10m '(rate(cloudburst_proxy_custom_total{key="incoming.HTTP/1.1"}[30s]) / 12)' > $1/http_requests_slo.py
  ...
  styx matplotlib --prometheus http://localhost:9090 --duration 10m 'sum(rate(example_sorting_requests_total[30s]))' > $1/http_requests_single_service.py
  echo "done!"
  sleep 120
}
...
# rate(go_memstats_alloc_bytes_total{instance="example-app:9997", job="example-app"}[30s]) / 5000000 (5mb)